...
### TODO: update PT info to more clearly flag unmet PT expectations. In plot, add shaded polygon. Possible table.
### TODO: dropdown menu to select a lab instead of requiring a click?
### TODO: flow chart development.
### TODO: shade colors to indicate FYH1 vs FYH2
...
ui <- page_navbar(
  theme = bs_theme(version = 5), # Use Bootstrap 5 theme
  nav_panel("Audits", "",
            navset_card_underline(
              title = "",
              nav_panel("Certification expirations", 
                        fluidRow(
                          leafletOutput("map_audits", width = "700px", height = "400px"), # Warning in min(exp_fys, na.rm = TRUE): no non-missing arguments to min; returning Inf
    ...
    "))
                          ...
                          DiagrammeR::grVizOutput("mermaidChart")
                        )
                        )
            )),
  nav_panel("Proficiency test data", "",
  navset_card_underline(
    title = "",
    nav_panel("Summarize by laboratory", 
              fluidRow(
                leafletOutput("map2", width = "700px", height = "400px"),
                column(width = 10, height = 9, plotOutput("plot2")),
                column(width = 8, DT::DTOutput("table2")),
                h3(""),
                h3("Download a pre-audit report for selected laboratory"),
                downloadButton("report", "Generate report")
              )),
    nav_panel("Summarize by method", #h2("Proficiency test results: summarized by method"),
              selectInput("method", "Select Method", choices = method_vec),
              fluidRow(leafletOutput("map", width = "700px", height = "400px"),
                       column(width = 8, DT::DTOutput("table1"))
              ))
  )),
nav_panel("Optional file upload", "File upload menu (optional)",
            dashboardPage(
            dashboardHeader(),
            dashboardSidebar(
            sidebarMenu(
...
")
    ...
    digraph {
      ...
      A [label = 'Start']
      B [label = 'Decision: Is x ≤ 10?']
      C [label = 'Do something']
      D [label = 'Do something else']
      E [label = 'End']
